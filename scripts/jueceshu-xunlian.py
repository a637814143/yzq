...
DEFAULT_FEATURES_PATH = Path(
    os.environ.get("DT_FEATURES_PATH", r"E:\\毕业设计\\新测试\\新的\\email_features.npy")
)
DEFAULT_LABELS_PATH = Path(
    os.environ.get("DT_LABELS_PATH", r"E:\\毕业设计\\新测试\\新的\\email_labels.txt")
)
...
def train_decision_tree_classifier(
    ...
    *,
    ...
    print(
        f"已选择 DecisionTreeClassifier，criterion={criterion}，max_depth={depth_text}，"
...
def save_model(model: DecisionTreeClassifier, model_output: Path | str) -> None:
    """将训练好的模型保存到指定路径。"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="使用决策树从特征和标签文件训练垃圾邮件分类模型",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "\n- 也可通过环境变量 DT_FEATURES_PATH 指定默认值"
        ),
    )
    parser.add_argument(
        "--labels-txt",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "\n- 也可通过环境变量 DT_MODEL_OUTPUT 指定默认值"
        ),
    )
    parser.add_argument(
        "--criterion",
        ...
        default="gini",
        help="划分质量评估指标（gini/entropy/log_loss）。",
    )
    parser.add_argument(
        "--max-depth",
        type=int,
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        model, y_valid, y_valid_pred, y_test, y_test_pred = train_decision_tree_classifier(
            X,
            y,
